function ChatMessage({ message }) {
  ...
  const renderText = (text) => {
    const html = text
      ...
      .replace(/```(.*?)```/gs, '<pre class="bg-gray-800 text-white p-2 rounded-md my-2 text-sm"><code>$1</code></pre>') // Code blocks
      .replace(/`(.*?)`/g, '<code class="bg-gray-200 text-red-500 px-1 rounded">$1</code>') // Inline code
      .replace(/\n/g, '<br />'); // New lines
    return { __html: html };
  }
    ...
    <div className={`flex ${isBot ? 'justify-start' : 'justify-end'} animate-fade-in`}>
      <div className={`flex items-end space-x-2 max-w-[80%] ${isBot ? '' : 'flex-row-reverse space-x-reverse'}`}>
        ...
        <div className={`rounded-2xl px-4 py-3 ${
          ...
          <div 
            className={`text-sm leading-relaxed prose ${isBot ? 'text-gray-700' : 'text-white'}`}
            dangerouslySetInnerHTML={renderText(message.text)}
          />
          <p className={`text-xs mt-2 text-right ${isBot ? 'text-gray-400' : 'text-white/70'}`}>
